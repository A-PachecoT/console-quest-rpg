...
def test_character_creation():
    character = Character(id=1, name="Hero", level=1, hp=100, max_hp=100, attack=10, defense=5)
...
def test_monster_creation():
    monster = Monster(id=1, name="Goblin", hp=50, attack=5, defense=2, experience_reward=10)
    assert monster.id == 1
    assert monster.name == "Goblin"
    assert monster.hp == 50
    assert monster.attack == 5
    assert monster.defense == 2
    assert monster.experience_reward == 10
